...
def generate_quests(db: Session, player: Player, day: str) -> dict:
    ...
    slots = []
    for q in defs:
        if not is_on_board(q, day):
            # A retired or parked quest keeps its QuestDef row — completions, streaks
            # and achievements key on it — but it never reaches the board, so
